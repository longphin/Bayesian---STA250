...
{
	# Note for non-R users: "invisible" can be used like a "return" statement if it's the last line of the function
...
{
	# Calculate logit^-1. The if statements are error checks (e.g. R gives exp(*)=NA if it is too large)
	try(z<-exp(X%*%theta.post))
	if(exists("z")==FALSE || class(z)=="try-error" || is.infinite(z) || is.na(z)) return(NA)
	ex=z/(1+z)
	if(exists("ex")==FALSE || class(ex)=="try-error" || is.infinite(ex) || is.na(ex)) return(NA)
			...
			verbose=TRUE)
{
	...
	for(i in 2:(burnin+niter))
	{
		...
		for(j in 1:p)
		{
			...
			# Note: Since the proposal is symmetric, I don't bother calculating it, so it is commented out.
			...
			if(log(runif(1)) < alpha)
			{
				...
				if(verbose==TRUE) print(sprintf("[%i, %i, %.2f] accepted with probability %.2f", i, j, theta.star[j], acceptance[i,j]))
			}else{
		...
		if(i<=burnin && retuneIt==0)
		{
			...
			for(k in 1:p)
			{
				...
				# variances are adjusted based on how far the acceptance rate is, targeting the [.3, .5] range. The adjustments are randomized a little.
